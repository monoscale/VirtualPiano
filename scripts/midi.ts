...
class VirtualPiano {
    ...
    private selectMIDIInputPort(): void {
        ...
        if (this.audioContext == null) {
            ...
            this.mainVolume.gain.value = 0.1;
    ...
    private processMIDIMessage(e: WebMidi.MIDIMessageEvent) {
        ...
        switch (command) {
            case 1: this.setModulation(data[2]); break;
    ...
    private setBend(highNibble: number, lowNibble: number) {
        ...
        for(var i = 0; i < 127; i++){
            this.oscillators[i].detune.setValueAtTime(bend, this.audioContext.currentTime);
        }
        //this.oscillator.detune.setValueAtTime(bend, this.audioContext.currentTime);
    }

    private setModulation(value: number) {
        //this.gain.setValueAtTime(value * 100 / 127, this.audioContext.currentTime);
    }
